...
| `/history` | Просмотр истории мероприятий |
| `/dbexport` | Экспорт всей базы данных как SQLite файл |

Админские команды автоматически показываются в меню бота для пользователей из `admin_ids` и в группе `admin_group_id` (если настроена). Обычные пользователи видят только `/start`.
